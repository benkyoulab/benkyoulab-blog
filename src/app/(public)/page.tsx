  ...
  return (
    <main className="flex-1">
      ...
      <section className="relative overflow-hidden">
        <span
          aria-hidden
          className="pointer-events-none absolute -top-8 -right-4 hidden select-none text-[11rem] leading-none font-bold text-red-50 md:block"
        >
          日本語
        </span>
        <div className="mx-auto max-w-5xl px-4 pt-16 pb-12 sm:pt-24 sm:pb-16">
          <FadeIn>
            <p className="inline-flex items-center gap-2 rounded-full bg-red-50 px-4 py-1.5 text-sm font-semibold text-red-600">
              <span>🎌</span> ブログ · Blog Belajar Bahasa Jepang
            </p>
          </FadeIn>
          <FadeIn delay={0.08}>
            <h1 className="mt-5 max-w-2xl text-4xl leading-tight font-bold tracking-tight text-gray-900 sm:text-5xl">
              ...
              <span className="relative inline-block text-red-600">
                ...
                <svg
                  aria-hidden
                  viewBox="0 0 220 10"
                  className="absolute -bottom-1 left-0 w-full text-red-200"
                  fill="none"
                >
            ...
            <dl className="mt-12 grid max-w-md grid-cols-3 gap-4">
              {[
                ...
                <div key={s.label} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100">
                  <dt className="order-2 text-xs text-gray-500">{s.label}</dt>
                  <dd className="text-2xl font-bold text-gray-900">{s.n}</dd>
                </div>
              ))}
            </dl>
          </FadeIn>
      ...
      {/* CHIP KATEGORI */}
      <section className="border-y border-gray-100 bg-white/70 backdrop-blur">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {catRows.map((c) => (
                <Link
                  ...
                  className="rounded-full border border-gray-200 bg-white px-4 py-1.5 text-sm text-gray-700 transition-all hover:border-red-200 hover:bg-red-50 hover:text-red-600 active:scale-95"
                >
                  {c.name}
      ...
      {/* DAFTAR ARTIKEL */}
      <section id="artikel" className="bg-gray-50 py-12">
        <div className="mx-auto max-w-5xl px-4">
          {items.length === 0 ? (
            ...
            <>
              <FadeIn>
                <h2 className="text-2xl font-bold tracking-tight text-gray-900">Artikel Terbaru</h2>
              </FadeIn>
                ...
                <nav className="mt-10 flex items-center justify-center gap-3">
                  {page > 1 && (
                    <Link
                      ...
                      className="rounded-full border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium hover:bg-gray-100"
                    >
                      ← Sebelumnya
                    </Link>
                  )}
